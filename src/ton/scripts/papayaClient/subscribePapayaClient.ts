...
export async function run(provider: NetworkProvider, args: string[]) {
  ...
  const wallet_address = Address.parse(
    args.length > 0 ? args[1] : await ui.input('creator`s wallet address')
  )

  const amount_per_second = parseInt(
    await ui.input('jetton amount per second'),
    10
  )
